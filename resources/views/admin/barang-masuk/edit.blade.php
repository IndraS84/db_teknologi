    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('admin.barang-masuk.update', $barangMasuk->id_barang_masuk) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        ...
                        <select class="form-control @error('id_produk') is-invalid @enderror" id="id_produk" name="id_produk" required>
                            ...
                            @foreach($produks as $produk)
                                <option value="{{ $produk->id_produk }}" {{ old('id_produk', $barangMasuk->id_produk) == $produk->id_produk ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('id_admin') is-invalid @enderror" id="id_admin" name="id_admin" required>
                            ...
                            @foreach($admins as $admin)
                                <option value="{{ $admin->id_admin }}" {{ old('id_admin', $barangMasuk->id_admin) == $admin->id_admin ? 'selected' : '' }}>
                                    {{ $admin->nama_admin }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control @error('tanggal_masuk') is-invalid @enderror" 
                                       id="tanggal_masuk" name="tanggal_masuk" value="{{ old('tanggal_masuk', $barangMasuk->tanggal_masuk) }}" required>
                                @error('tanggal_masuk')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
