    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('admin.barang-keluar.update', $barangKeluar->id_barang_keluar) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        ...
                        <select class="form-control @error('id_produk') is-invalid @enderror" id="id_produk" name="id_produk" required>
                            ...
                            @foreach($produks as $produk)
                                <option value="{{ $produk->id_produk }}" {{ old('id_produk', $barangKeluar->id_produk) == $produk->id_produk ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('id_pelanggan') is-invalid @enderror" id="id_pelanggan" name="id_pelanggan" required>
                            ...
                            @foreach($pelanggans as $pelanggan)
                                <option value="{{ $pelanggan->id_pelanggan }}" {{ old('id_pelanggan', $barangKeluar->id_pelanggan) == $pelanggan->id_pelanggan ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('id_admin') is-invalid @enderror" id="id_admin" name="id_admin" required>
                            ...
                            @foreach($admins as $admin)
                                <option value="{{ $admin->id_admin }}" {{ old('id_admin', $barangKeluar->id_admin) == $admin->id_admin ? 'selected' : '' }}>
                                    {{ $admin->nama_admin }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @error('jumlah') is-invalid @enderror" 
                                       id="jumlah" name="jumlah" value="{{ old('jumlah', $barangKeluar->jumlah) }}" min="1" required>
                                @error('jumlah')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
